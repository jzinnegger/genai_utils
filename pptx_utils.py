...
def change_table_style(shape, style_name = 'NoStyleTableGrid'):
    """
    Changes the style of a table based on the style name.

    Args:
    shape: A shape object that is a table.
    style_name: The name of the style to apply to the table.
    """
    ...
    style_id = table_style_dict.get(style_name)

    if style_id is not None and shape is not None and hasattr(shape, '_element') and shape._element.graphic.graphicData.tbl is not None:
        tbl = shape._element.graphic.graphicData.tbl
